...
### Added
- `server.json` declaring the package for the official [MCP Server Registry](https://registry.modelcontextprotocol.io/) (`io.github.RonieNeubauer/cli2mcp`).
- `mcpName` field in `package.json` (required by the registry to verify package ownership).

### Notes
- 0.1.3 was tagged but never reached npm. 0.1.4 is the first release with the registry metadata; both 0.1.3 and 0.1.4 carry the same code changes — the bump is purely to keep the npm and registry versions aligned.
...
[Unreleased]: https://github.com/RonieNeubauer/cli2mcp/compare/v0.1.4...HEAD
[0.1.4]: https://github.com/RonieNeubauer/cli2mcp/compare/v0.1.3...v0.1.4
