...
describe("parseArgs", () => {
  test("parses positional command", () => {
    const opts = parseArgs(["node", "mcpify", "jq"]);
  ...
  test("--name overrides tool name", () => {
    const opts = parseArgs(["node", "mcpify", "jq", "--name", "json-query"]);
    expect(opts.name).toBe("json-query");
  });

  test("--description sets description", () => {
    const opts = parseArgs(["node", "mcpify", "jq", "--description", "JSON processor"]);
    expect(opts.description).toBe("JSON processor");
  });

  test("--timeout parses as number", () => {
    const opts = parseArgs(["node", "mcpify", "jq", "--timeout", "30000"]);
    expect(opts.timeout).toBe(30000);
  });

  test("--cwd sets working directory", () => {
    const opts = parseArgs(["node", "mcpify", "jq", "--cwd", "/tmp"]);
    expect(opts.cwd).toBe("/tmp");
  });

  test("--env accumulates repeated flags", () => {
    const opts = parseArgs(["node", "mcpify", "jq", "--env", "FOO=bar", "--env", "BAZ=qux"]);
  ...
  test("--stderr accept include | drop | error", () => {
    for (const mode of ["include", "drop", "error"] as const) {
      const opts = parseArgs(["node", "mcpify", "jq", "--stderr", mode]);
      expect(opts.stderr).toBe(mode);
    }
  });

  test("--stderr invalid value exits with code 1", () => {
    expect(() => parseArgs(["node", "mcpify", "jq", "--stderr", "garbage"])).toThrow();
  });
});
